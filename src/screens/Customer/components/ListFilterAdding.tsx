...
const ListFilterAdding = (props: Props) => {
  ...
  const [curListRoute, setCurListRoute] =
    useState<ListCustomerRoute[]>(listRoute);
  ...
  const handleRouter = useCallback(
    (item: any) => {
      // @ts-ignore
      const newData = curListRoute?.map(itemRoute => {
        if (itemRoute.name === item.name) {
          return {...itemRoute, isSelected: !itemRoute?.isSelected};
        } else {
          return itemRoute;
        }
      });
      setCurListRoute(newData);

      // @ts-ignore
      setData(prevState => ({
        ...prevState,
        router: newData.filter(itemNew => itemNew.isSelected),
      }));
    },
    [data, setValueFilter, filterRef, curListRoute],
  );

  useEffect(() => {
    if (listRoute?.length > 0 && curListRoute?.length === 0) {
      setCurListRoute(listRoute);
    }
  }, [listRoute]);
    ...
    <Block>
      ...
      type === AppConstant.CustomerFilterType.tuyen ? (
        <Block>
          ...
          {curListRoute?.length > 0 &&
            curListRoute.map((item: any) => {
              return (
                <TouchableOpacity
                  style={styles.containItemBottomView}
                  key={item?.channel_code}
                  onPress={() => handleRouter(item)}>
                  <Text style={{marginVertical: 8}}>{item.channel_name}</Text>
                  {item.isSelected && (
                    <AppIcons
                      iconType={AppConstant.ICON_TYPE.Feather}
                      name="check"

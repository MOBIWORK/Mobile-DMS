        ...
        switch (type) {
            ...
            case "remove":
                if (favourites.length > 1) {
                    ...
                    const newArr = widgets.map(item2 =>item2.id === item.id ? {...item2,isUse :false} : item2)
                    setWidgets(newArr)
                }
                break;
            default:
        ...
        <MainLayout style={styles.layout}>
            <AppContainer>
                <View style={styles.container}>
                    <AppHeader label={getLabel("Tuỳ chỉnh tiện ích")} onBack={() => navigation.goBack()} />
                    <View>
                        <View style={[styles.flexSpace]}>
                            <Text style={[styles.titleView]}>{getLabel("Tiên ích yêu thích")}</Text>
                            <Text style={[styles.action]}>
                                {isEdit ?
                                    <Text onPress={() => onSave()}>{getLabel("Lưu lại")}</Text>
                                    :
                                    <Text onPress={() => setEdit(!isEdit)}>{getLabel("Chỉnh sửa")}</Text>}
                            </Text>
                        </View>
                        {isEdit && (
                            <View style={styles.viewDesc}>
                                <Text style={[styles.descAction]}>{getLabel("Nhấn giữ và kéo để di chuyển tiện ích")}</Text>
                            </View>
                        )}
                                ...
                                {favourites &&
                                    favourites.map((item) => (
                                        <View key={item.id} style={styles.viewItem}>
                                            ...
                                            />
                                            {isEdit ? (
                                                <Pressable onPress={() => changeUtilltiesFavourites("remove", item)} style={styles.positionBt}>
                                                    <AppIcons
                                                        iconType="AntIcon"
                                                        name="minuscircle"
                                                        size={16}
                            ...
                            <View style={[styles.containerItem]}>
                                {widgets &&
                                    widgets.map((item) => (
                                        <View
                                            key={item.id}
                                            style={[styles.viewItem,{opacity: item.isUse && isEdit ? 0.7 : 1}]}
                                        >
                                            <ItemuWidget
                                                name={item.name}
                                                source={item.icon}
                                            ...
                                            {isEdit ? (
                                                <View style={styles.positionBt}>
                                                    {item.isUse ? <AppIcons
                                                        onPress={() => changeUtilltiesFavourites("remove", item)}
                                                        iconType="AntIcon"
                                                        name="minuscircle"
                                                        size={16}
                                                        color={colors.text_disable}
                                                    /> : <AppIcons
                                                        onPress={() => changeUtilltiesFavourites("add", item)}
                                                        iconType="IonIcon"
                                                        name="add-circle"
                                                        size={20}
